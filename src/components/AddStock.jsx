...
const AddStock = () => {
  ...
  const fetchFabricKgAndMeterRelationShip = async (fabric_number) => {
    ...
    try {
      ...
      const response = await axios.get(
        `${BASE_URL}/api/v1/relation/details?fabric_number=${fabric_number}`
      );
  ...
  const fetchStore2Stock = async (fabric_number) => {
    ...
    try {
      ...
      if (!data?.availableStock || Number(data?.availableStock) <= 0) {
        alert(
          `${formData.fabricNumber} fabric number or stock not found in store 2. Please add new.`
        );
        setFormData({
          ...formData,
          fabric_source: '',
  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.fabric_source === 'Store2') {
      confirmAction = window.confirm(
        'Are you sure you want to add stock and remove the current stock from Store 2?'
      );
      if (!confirmAction) {
        setFormData({
          fabricNumber: '',
    ...
    try {
      const { fabricNumber, availableStock, location, meter_unit, kg_unit, fabric_source } =
        formData;
      ...
      if (fabric_source === 'Store2') {
        ...
        updatePayload.availableStock =
          Number(existingStock.availableStock || 0) +
          Number(availableStock) * Number(relationDetails.fabric_in_meter);
      } else {
        ...
        try {
          ...
          updatePayload.availableStock =
            Number(existingStock.availableStock || 0) + Number(meter_unit);
        } catch (error) {
  ...
  useEffect(() => {
    if (
      formData.fabricNumber?.toString().length === 4 ||
      formData.fabricNumber?.toString().length === 3
    ) {
  ...
  const showKgMeterInputs =
    !showKgMeterRelationship && formData.fabric_source && formData.fabric_source !== 'Store2';
  const showQuantityInput =
    (formData.fabric_source === 'Vendor' || !formData.fabric_source) && !stock2?.availableStock > 0;
      ...
      <div className="grid grid-cols-2 gap-10">
        ...
        <div className="left">
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-2 gap-4 items-center">
            {/* Fabric Number */}
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Fabric Number</label>
              <input
                type="number"
                name="fabricNumber"
            ...
            {showKgMeterInputs && (
              <>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Kg Unit</label>
                  <input
                    type="number"
                    name="kg_unit"
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Meter Unit</label>
                  <input
                    type="number"
                    name="meter_unit"
            ...
            {showQuantityInput && (
              <div className="col-span-2">
                ...
                <input
                  ...
                  onChange={handleChange}
                  placeholder={
                    relationDetails.fabric_in_meter > 0
                      ? 'Enter quantity in KG'
                      : 'Enter quantity in Meter'
                  }
                  className="w-full px-4 py-2 bg-gray-100 rounded-md outline-gray-200"
                  required
                />
            ...
            {formData.fabric_source === 'Store2' && stock2?.availableStock > 0 && (
              <div className="col-span-2 p-3 bg-blue-50 rounded-md">
                <p className="text-sm font-medium text-gray-700">
                  ...
                  <span className="font-bold">{stock2.availableStock} KG</span>
                </p>
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center mt-10 gap-2">
            ...
            <p className="text-center text-sm text-gray-600">Product image loading...</p>
          </div>
        ) : (
          product?.style_id && (
